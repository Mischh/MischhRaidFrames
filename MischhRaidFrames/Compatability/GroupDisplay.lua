...
function GroupDisplay:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("Compatability\\GroupDisplay.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

do
	local f = MRF.OnLoad
	function MRF:OnLoad(...) --hook into MRF-Addon Loading to imitate a load on this one.
		f(self, ...)
		GroupDisplay:OnLoad(...)
	end
end
...
function GroupDisplay:OnGroupUpdated()
	...
	if self.bDisplayedRaid == nil and GroupLib.InRaid() then
		...
		ChatSystemLib.PostOnChannel(self.eChatChannel, Apollo.GetString("Group_BecomeRaid"), "")
	end
end
