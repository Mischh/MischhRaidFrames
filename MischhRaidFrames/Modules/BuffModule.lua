...
function BuffMod:iconUpdate(frame, unit)
	...
	for _, buff in ipairs(unitBuffs.arBeneficial) do
...
function BuffMod:InitIconSettings(parent)
	local L = MRF:Localize({--English
		...
		["Only applied by myself"] = "Nur von mir verteilte Buffs",
	...
	def_selRow:SetAnchorOffsets(def_tab:FindChild("CheckboxActivated"):GetAnchorOffsets())
		...
		--Selecting Dropdown
	local selector, selected = MRF:SelectRemote("Buffs_SelectedBuff", buffTblOpt)
	local selection = {} 
	...
	local mineOpt = MRF:GetOption(selected, "onlyMine")
	MRF:applyCheckbox(checkRow:FindChild("Left"), isSpOpt, L["Is SpellID"])
	MRF:applyCheckbox(checkRow:FindChild("Right"), mineOpt, L["Only applied by myself"])
